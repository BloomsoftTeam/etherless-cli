...
const msgLinkOrCreate = chalk.white.bold("Do you want to link an existing ETH wallet or create a new one? [link/create]");
...
.command('init', msgInit, () => {}, (argv) => {
    ...
    const msg = boxen(msgLinkOrCreate, boxLinkorCreate );
    

    rl.question(msg, (answer1) => {
...
.command('run <funcName> [params..]', 'run a function', () => {}, async (argv) => {
    try {
        ...
        const contractRun = ethers.createContract("0x38bB51CaAD409943d4dF3A177674B03262C10F98").connect(ethers.userWallet); //per testare
